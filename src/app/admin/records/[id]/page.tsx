...
function Row({ label, initial, min, max, final, unit = "" }: {
  ...
  const finalColor = inSpec ? "#16a34a" : outOfSpec ? "#dc2626" : "#f97316";
  ...
  return (
    <div className="grid text-xs border-b border-gray-50 last:border-0" style={{ gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr" }}>
      ...
      <div className="px-2 py-2 text-center text-gray-500">{fmt(initial)}</div>
...
export default async function RecordDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const sa = db.select().from(secondaryAngles).where(eq(secondaryAngles.recordId, recordId)).get();

  const { alignment_records: rec, vehicles: veh, users: tech } = row;
  const vehicleLabel = [veh?.make, veh?.model, veh?.yearFrom ? `${veh.yearFrom}${veh.yearTo ? `–${veh.yearTo}` : ""}` : null].filter(Boolean).join(" ") || "—";
    ...
    <div className="max-w-2xl" style={{ fontFamily: "system-ui, sans-serif" }}>
      ...
      {pa && (
        <>
          <Section title="ÖN — KASTET"><TableHeader />
            <Row label="Sol" initial={pa.frontCasterLeftInitial} min={pa.frontCasterLeftMin} max={pa.frontCasterLeftMax} final={pa.frontCasterLeftFinal} unit="°" />
            <Row label="Sağ" initial={pa.frontCasterRightInitial} min={pa.frontCasterRightMin} max={pa.frontCasterRightMax} final={pa.frontCasterRightFinal} unit="°" />
          </Section>
          <Section title="ÖN — KAMBER"><TableHeader />
            <Row label="Sol" initial={pa.frontCamberLeftInitial} min={pa.frontCamberLeftMin} max={pa.frontCamberLeftMax} final={pa.frontCamberLeftFinal} unit="°" />
            <Row label="Sağ" initial={pa.frontCamberRightInitial} min={pa.frontCamberRightMin} max={pa.frontCamberRightMax} final={pa.frontCamberRightFinal} unit="°" />
          </Section>
          <Section title="ÖN — ROT (mm)"><TableHeader />
            <Row label="Sol" initial={pa.frontToeLeftInitial} min={pa.frontToeLeftMin} max={pa.frontToeLeftMax} final={pa.frontToeLeftFinal} unit="mm" />
            <Row label="Sağ" initial={pa.frontToeRightInitial} min={pa.frontToeRightMin} max={pa.frontToeRightMax} final={pa.frontToeRightFinal} unit="mm" />
            <Row label="Toplam" initial={pa.frontToeTotalInitial} min={pa.frontToeTotalMin} max={pa.frontToeTotalMax} final={pa.frontToeTotalFinal} unit="mm" />
          </Section>
          <Section title="ARKA — KAMBER"><TableHeader />
            <Row label="Sol" initial={pa.rearCamberLeftInitial} min={pa.rearCamberLeftMin} max={pa.rearCamberLeftMax} final={pa.rearCamberLeftFinal} unit="°" />
            <Row label="Sağ" initial={pa.rearCamberRightInitial} min={pa.rearCamberRightMin} max={pa.rearCamberRightMax} final={pa.rearCamberRightFinal} unit="°" />
          </Section>
          <Section title="ARKA — ROT (mm)"><TableHeader />
            <Row label="Sol" initial={pa.rearToeLeftInitial} min={pa.rearToeLeftMin} max={pa.rearToeLeftMax} final={pa.rearToeLeftFinal} unit="mm" />
            <Row label="Sağ" initial={pa.rearToeRightInitial} min={pa.rearToeRightMin} max={pa.rearToeRightMax} final={pa.rearToeRightFinal} unit="mm" />
            <Row label="Toplam" initial={pa.rearToeTotalInitial} min={pa.rearToeTotalMin} max={pa.rearToeTotalMax} final={pa.rearToeTotalFinal} unit="mm" />
          </Section>
          <Section title="ARKA — İTİŞ AÇISI"><TableHeader />
            <Row label="İtiş Açısı" initial={pa.thrustAngleInitial} final={pa.thrustAngleFinal} unit="°" />
          </Section>
        </>
      )}
      ...
      {sa && (
        <Section title="S.A.İ."><TableHeader />
          <Row label="Sol" initial={sa.saiLeftInitial} min={sa.saiLeftMin} max={sa.saiLeftMax} final={sa.saiLeftFinal} unit="°" />
          <Row label="Sağ" initial={sa.saiRightInitial} min={sa.saiRightMin} max={sa.saiRightMax} final={sa.saiRightFinal} unit="°" />
        </Section>
      )}
    </div>
  );
